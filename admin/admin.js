...
const api = async (path, options = {}) => {
  let response;
  try {
    response = await fetch(`${apiBase}/api/admin/${path}`, { credentials: "include", ...options });
  } catch (error) {
...
function showUserForm(user = {}) {
  $("#user-editor")?.remove();
  $("#resource-panel .resource-card").insertAdjacentHTML("afterbegin", `<form id="user-editor" class="editor-form"><h3>${user.id ? "Edit admin user" : "Create admin user"}</h3><label>Full name<input name="full_name" value="${escapeHtml(user.full_name || "")}" required></label><label>Email<input name="email" type="email" value="${escapeHtml(user.email || "")}" required></label>${user.id ? "" : '<label>Temporary password<div class="password-field"><input name="password" type="password" minlength="6" required autocomplete="new-password"><button class="password-toggle" type="button" data-password-target="user-password" aria-label="Show password" title="Show password"><i class="fa-regular fa-eye"></i></button></div></label>'}<label>Role<select name="role"><option value="super_admin" ${user.role === "super_admin" ? "selected" : ""}>Super Admin</option><option value="editor" ${user.role === "editor" ? "selected" : ""}>Editor</option><option value="viewer" ${user.role === "viewer" ? "selected" : ""}>Viewer</select></label><label>Status<select name="status"><option value="active" ${user.status !== "inactive" ? "selected" : ""}>Active</option><option value="inactive" ${user.status === "inactive" ? "selected" : ""}>Inactive</option></select></label><button class="primary-button" type="submit">Save user</button></form>`);
  const passwordInput = $("#user-editor input[name=password]"); if (passwordInput) passwordInput.id = "user-password";
...
function renderChangePassword() {
  $("#resource-panel").innerHTML = `<div class="resource-card"><div class="resource-card-header"><div><h2>Change Password</h2><p>Use a password with at least 6 characters, upper/lowercase letters, and a number.</p></div></div><form id="change-password-form" class="editor-form"><label>Current password<div class="password-field"><input name="current_password" type="password" autocomplete="current-password" required><button class="password-toggle" type="button" data-password-target="current-password" aria-label="Show password" title="Show password"><i class="fa-regular fa-eye"></i></button></div></label><label>New password<div class="password-field"><input id="new-password" name="new_password" type="password" minlength="6" autocomplete="new-password" required><button class="password-toggle" type="button" data-password-target="new-password" aria-label="Show password" title="Show password"><i class="fa-regular fa-eye"></i></button></div></label><label>Confirm new password<div class="password-field"><input id="confirm-password" name="confirm_password" type="password" minlength="6" autocomplete="new-password" required><button class="password-toggle" type="button" data-password-target="confirm-password" aria-label="Show password" title="Show password"><i class="fa-regular fa-eye"></i></button></div></label><button class="primary-button" type="submit">Change password</button></form></div>`;
  $("#change-password-form").addEventListener("submit", async (event) => { event.preventDefault(); try { await api("change-password", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(Object.fromEntries(new FormData(event.target).entries())) }); flash("Password changed. Other sessions were signed out."); event.target.reset(); } catch (error) { flash(error.message, "error"); } }); bindPasswordToggles();
}
function bindPasswordToggles() { document.querySelectorAll(".password-toggle").forEach((button) => button.addEventListener("click", () => { const input = button.previousElementSibling; const visible = input.type === "text"; input.type = visible ? "password" : "text"; button.setAttribute("aria-label", visible ? "Show password" : "Hide password"); button.title = visible ? "Show password" : "Hide password"; button.innerHTML = `<i class="fa-regular fa-eye${visible ? "" : "-slash"}"></i>`; })); }
...
function showResetForm(token) {
  ...
  $(".login-card").insertAdjacentHTML("beforeend", '<form id="reset-form"><p class="login-copy">Choose a new password for your admin account.</p><label for="reset-password">New password</label><div class="password-field"><input id="reset-password" type="password" minlength="6" autocomplete="new-password" required><button class="password-toggle" type="button" aria-label="Show password" title="Show password"><i class="fa-regular fa-eye"></i></button></div><label for="reset-confirm">Confirm new password</label><div class="password-field"><input id="reset-confirm" type="password" minlength="6" autocomplete="new-password" required><button class="password-toggle" type="button" aria-label="Show password" title="Show password"><i class="fa-regular fa-eye"></i></button></div><button class="primary-button full-width" type="submit">Reset password</button><div id="reset-message" class="alert d-none"></div></form>');
  bindPasswordToggles();
